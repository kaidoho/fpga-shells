...
import sifive.fpgashells.ip.xilinx.artys750mig.{ARTYS750MIGIOClocksReset, ARTYS750MIGIODDR, artyS750mig}

case class XilinxArtyS7MIGParams(
  address : Seq[AddressSet]
)

class XilinxArtyS7MIGPads(depth : BigInt) extends ARTYS750MIGIODDR(depth) {
  def this(c : XilinxArtyS7MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxArtyS7MIGIO(depth : BigInt) extends ARTYS750MIGIODDR(depth) with ARTYS750MIGIOClocksReset

class XilinxArtyS7MIGIsland(c : XilinxArtyS7MIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  require((depth<=0x10000000L),"artyS750mig supports up to 256MB depth configuraton")
  ...
  lazy val module = new LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxArtyS7MIGIO(depth)
    })
    ...
    val blackbox = Module(new artyS750mig(depth))
...
class XilinxArtyS7MIG(c : XilinxArtyS7MIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxArtyS7MIGIsland(c, crossing))
